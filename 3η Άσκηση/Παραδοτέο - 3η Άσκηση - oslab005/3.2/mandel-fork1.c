...
void compute_and_output_mandel_line(int fd, int init_line, int procs)
{
	/*
	...
	for(int line = init_line; line < y_chars; line += procs) {
		...
		if(sem_wait(&sem[line % procs]) < 0) {perror("sem_wait"); exit(1);}
	        output_mandel_line(fd, color_val);
		if(sem_post(&sem[(line+1) % procs]) < 0) {perror("sem_post"); exit(1);}
	}
}
...
int main(int argc, char *argv[])
{
	...
	sem = create_shared_memory_area(procs * sizeof(sem_t));
	if(sem_init(&sem[0], 1, 1) < 0) {perror("sem_init"); exit(1);}
	for(int i = 1; i < procs; i++)
		if(sem_init(&sem[i], 1, 0) < 0) {perror("sem_init"); exit(1);}
	...
	for (int init_line = 0; init_line < procs; init_line++) {
		...
		if(p == 0) {
                	compute_and_output_mandel_line(1, init_line, procs);
                	exit(0);
               	}
	}
	...
	for(int i=0; i<procs; i++)
		wait(NULL);
	destroy_shared_memory_area(sem, procs*sizeof(sem_t));
