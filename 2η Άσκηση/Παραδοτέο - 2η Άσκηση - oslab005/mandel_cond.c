 ...
 * mandel_cond.c
 *
 * A program to draw the Mandelbrot Set on a 256-color xterm, with condition variables.
 *
 */
...
void *compute_and_output_mandel_line(void *arg)
{
	...
	for(int line = thr->thrid; line < y_chars; line += thr->thrcnt) {
		...
		if(line != counter) pthread_cond_wait(&cond[thr->thrid], &lock);
		counter++;
		output_mandel_line(1, thr->color_val);
		pthread_cond_broadcast(&cond[(line+1) % thr->thrcnt]);
...
int main(int argc, char *argv[])
{
	...
	/*
        ...
        if (argc != 2)
            usage(argv[0]);
        if (safe_atoi(argv[1], &thrcnt) < 0 || thrcnt <= 0 || argv[1][0] == '1') {
	...
	cond = safe_malloc(thrcnt * sizeof(pthread_cond_t));
